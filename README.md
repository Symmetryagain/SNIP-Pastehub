...
Snip Share 不仅仅是一个共享剪切板。它支持完整的 Markdown 语法、复杂的数学公式渲染，并引入了“Fork”机制，允许成员在不破坏原帖的情况下进行知识演进。

## ✨ 核心特性

- **极致排版**：采用 Inter + 霞鹜文楷 + JetBrains Mono 字体栈，专为技术阅读优化。
- **数学公式**：完美支持 LaTeX 行内及块级公式渲染（基于 KaTeX/MathJax）。
- **Fork 机制**：一键派生他人的帖子，支持思路的并行演进。
- **墓碑模式 (Tombstone)**：作者删除帖子后保留其“存在痕迹”，确保派生贴的溯源链条完整。
...
- 数据备份：
直接备份 `snip_data/snip_data.db` 文件即可。
